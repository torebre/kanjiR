...
rows <- sum(sapply(input.data, function(x) {
  dim(x)[1]
}))
number.of.rows.limit <- rows * number.of.closest.lines.to.include
lines <- matrix(nrow = number.of.rows.limit, ncol = 8)
...
for(i in 1:length(input.data)) {
  ...
  current.data.as.list <- as.list(current.data)
  
  # Only use lines that have a length greater than line.length.cutoff
  number.of.lines.in.kanji <- dim(current.data)[1]
  ...
  for(j in 1:number.of.lines.in.kanji) {
    ...
    closest.lines <- ExtractClosestLinesToLine2(current.line["start_x"], current.line["start_y"], stop.1.x, stop.1.y, number.of.closest.lines.to.include, current.data[-j, ])
    ...
    current.line.as.list <- as.list(current.line)
    closest.lines.in.kanji <- current.data[which(current.data[ , 2] %in% closest.lines), ]
    lines.draw <- ExtractRelativePositions(current.line.as.list, as.data.frame(closest.lines.in.kanji), c(), 
                                           j, use.relative.distance = F)
    
    # print(dim(lines.draw))

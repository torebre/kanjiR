DrawLinePair <- function(relative.length, angle.diff, start.pair.difference, start.pair.angle.diff) {
  line1.length <- 10
  ...
  line2.end.x <- round(line2.length * cos(angle.diff))
  line2.end.y <- round(start.pair.difference * sin(start.pair.angle.diff))
  
  DrawLines(list(CreateLine(line1.start.x, line1.start.y, line1.end.x, line1.end.y), 
